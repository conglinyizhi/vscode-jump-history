...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('jumpHistory.goToLocation', async (location: { uri: vscode.Uri; range: vscode.Range }) => {
...
async function checkAndRecordJump(uri: vscode.Uri, range: vscode.Range) {
    ...
    debounceTimer = setTimeout(async () => {
        ...
        try {
            const isGoRelated =
                lastPosition!.uri.fsPath.endsWith('.go') ||
                uri.fsPath.endsWith('.go') ||
                uri.toString().includes('.go');
            ...
            const isTarget = results.some((locations: (vscode.Location[] | vscode.LocationLink[]) | undefined) =>
                locations && locations.length > 0 && locations.some((loc: vscode.Location | vscode.LocationLink) => locationMatches(loc, uri, range))
            );
...
function locationMatches(
    ...
    if ('targetUri' in loc) {
        return (
            loc.targetUri.toString() === uri.toString() &&
            loc.targetRange.start.line === range.start.line
        );
    }
