...
插件通过监听 `onDidChangeActiveTextEditor` 和 `onDidChangeTextEditorSelection` 事件，在检测到显著的位置变化后，利用 VSCode 内置的 `vscode.executeDefinitionProvider` 反推该变化是否由一次 "Go to Definition" 操作引起。如果是，则将 `from → to` 的记录压入堆栈并在侧边栏中渲染。
